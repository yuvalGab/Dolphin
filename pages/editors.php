<?php
    ...
    if(isset($_COOKIE['permission']) && $_COOKIE['permission'] = 'admin') {
        $isAdmin = true;
        $sth = $db->prepare("SELECT * FROM contents ORDER BY contentID DESC");
    } else {
        $sth = $db->prepare("SELECT * FROM contents WHERE username = '$username' ORDER BY contentID DESC");
    }
...
<body>
    <div class="wrapper">
        ...
        <div class="main">
            ...
            <div class="contents">
                <?php if($isAdmin) : ?>
                    <p id="guiding">שלום <?= $username ?>, הנך נמצא במשתמש עם הרשאות של מנהל, ביכולתך להוסיף תוכן משלך ולערוך תוכן של משתמשים אחרים. </p>
                <?php else : ?>
                    <p id="guiding">שלום <?= $username ?>, כאן ניתן להוסיף תוכן חדש ולערוך את התוכן שלך. </p>
                <?php endif; ?>
                <hr />
                <button id="add-content" class="button">הוסף תוכן חדש</button>
                <hr />
